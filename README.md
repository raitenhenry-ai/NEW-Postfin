...
2. **Script** — OpenAI reads the product page *and the product photos*, then
   writes the five UGC beats (hook / problem / solution / demo / CTA) as
   spoken lines plus a storyboard: one shot direction per line, and which
   photo shows the item in that shot. Tone (casual / hyped / pro /
   storytelling) and style (Product POV, GRWM, Unboxing, Before-After, Demo)
   steer it. Falls back to templates without a key.
3. **Render** — either **Kling** (default), which generates each shot with
   the real product photos as references so the item appears in scenes it was
   never photographed in, or the **HeyGen avatar API** for a talking-creator
   video. Kling's silent shots are stitched by ffmpeg with burned-in captions
   and an OpenAI TTS voiceover. Both output 1080×1920. See
   [Product videos with Kling](#product-videos-with-kling).
...
Requirements: Node 18+, ffmpeg + ffprobe on PATH (they stitch the generated
shots together and burn in the captions).

## Product videos with Kling

Set `KLING_ACCESS_KEY` and `KLING_SECRET_KEY` and each storyboard shot is
generated by Kling 2.1 Standard, then stitched into one 1080×1920 video with
captions and a voiceover.

What makes this different from feeding the photos in as slides: every shot
that contains the product is submitted with **all** the product photos as
references, not as its first frame. The model keeps the real item — its
shape, colour, label — and places it into whatever the shot direction
describes: held in someone's hand, on a cluttered counter, going into a gym
bag. Shots with no product in them are generated from the direction alone.
A shot the script marked product-free but that names the product anyway is
promoted back to a reference shot, so the generator can't invent a
different-looking item.

Credentials come from the Kling console as an **Access Key / Secret Key**
pair; the app signs a short-lived JWT per request from them. `KLING_MODE`
switches `std` (Standard, the default) to `pro`, `KLING_SHOT_SECONDS` is 5 or
10 per shot, and `KLING_MAX_SHOTS` caps how many shots one video pays for
(default 5, so a 25-second video).

`KLING_MODEL` and `KLING_API_BASE` are overridable because Kling renames
model ids between releases and runs regional endpoints — a change there is a
config edit, not a redeploy of new code.

## Avatar videos with HeyGen

Set `HEYGEN_API_KEY` and the video is instead rendered as a talking avatar
speaking the generated script, at 1080×1920. `VIDEO_PROVIDER` forces one or
the other (`auto` | `kling` | `heygen`); `auto` means "Kling when its keys
are set, otherwise HeyGen".
...
it's the avatar speaking. The Kling path is the one that burns captions in.
